...
@Component({
  ...
  templateUrl: './merienda.page.html',
  styleUrls: ['./merienda.page.scss'],
})
export class MeriendaPage implements OnInit {
  ...
  descripciones1() {
    firebase.auth().onAuthStateChanged((user) => {
      ...
      var docRef = this.db.collection("users").doc(uid).collection("Comidas").doc("Merienda");
  ...
  Tonificar() {
    this.about = (comida.tonificar[3].about);
    this.about2 = (comida.tonificar[3].about2);
  }
  Adelgazar() {
    this.about = (comida.adelgazar[3].about);
    this.about2 = (comida.adelgazar[3].about2);

  }
  Musculacion() {
    this.about = (comida.muscular[3].about);
    this.about2 = (comida.muscular[3].about2);
  }
  ...
  guardarKcal() {
    firebase.auth().onAuthStateChanged((user) => {
      const uid = user.uid;
      var docRef = this.db.collection("users").doc(uid);
      this.db.collection('users').doc(uid).collection("Kcal").doc("Merienda").update({
  ...
  editarBase2() {
    firebase.auth().onAuthStateChanged((user) => {
      const uid = user.uid;
      var docRef = this.db.collection("users").doc(uid);
      this.db.collection('users').doc(uid).collection("Comidas").doc("Merienda").update({
