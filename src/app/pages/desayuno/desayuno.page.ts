...
export class DesayunoPage implements OnInit {
  ...
  getAllMacros(){
    firebase.auth().onAuthStateChanged((user) => {
      ...
      var docRef = this.db.collection("users").doc(uid).collection("Macros").doc("Desayunp");

      docRef.get().subscribe((doc) => {
        if (doc.exists) {
          this.proteinas= doc.get("Proteinas");
  ...
  descripciones1() {
    firebase.auth().onAuthStateChanged((user) => {
      ...
      var docRef = this.db.collection("users").doc(uid).collection("Comidas").doc("Desayuno");
  ...
  Tonificar() {
    this.about = (comida.tonificar[0].about);
    this.about2 = (comida.tonificar[0].about2);
  }
  Adelgazar() {
    this.about = (comida.adelgazar[0].about);
    this.about2 = (comida.adelgazar[0].about2);

  }
  Musculacion() {
    this.about = (comida.muscular[0].about);
    this.about2 = (comida.muscular[0].about2);
  }
  ...
  guardarKcal() {
    firebase.auth().onAuthStateChanged((user) => {
      const uid = user.uid;
      var docRef = this.db.collection("users").doc(uid);
      this.db.collection('users').doc(uid).collection("Kcal").doc("Desayuno").update({
  ...
  editarBase2() {
    firebase.auth().onAuthStateChanged((user) => {
      const uid = user.uid;
      var docRef = this.db.collection("users").doc(uid);
      this.db.collection('users').doc(uid).collection("Comidas").doc("Desayuno").update({
