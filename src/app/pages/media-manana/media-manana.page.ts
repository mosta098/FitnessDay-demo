...
export class MediaMananaPage implements OnInit {
  ...
  descripciones1() {
    firebase.auth().onAuthStateChanged((user) => {
      ...
      var docRef = this.db.collection("users").doc(uid).collection("Comidas").doc("Media_Mañana");
  ...
  guardarKcal() {
    firebase.auth().onAuthStateChanged((user) => {
      const uid = user.uid;
      var docRef = this.db.collection("users").doc(uid);
      this.db.collection('users').doc(uid).collection("Kcal").doc("Media_Mañana").update({
  ...
  editarBase2() {
    firebase.auth().onAuthStateChanged((user) => {
      const uid = user.uid;
      var docRef = this.db.collection("users").doc(uid);
      this.db.collection('users').doc(uid).collection("Comidas").doc("Media_Mañana").update({
