...
export class CenaPage implements OnInit {
  ...
  descripciones1() {
    firebase.auth().onAuthStateChanged((user) => {
      ...
      var docRef = this.db.collection("users").doc(uid).collection("Comidas").doc("Cena");
  ...
  Tonificar() {
    this.about = (comida.tonificar[4].about);
    this.about2 = (comida.tonificar[4].about2);
  }
  Adelgazar() {
    this.about = (comida.adelgazar[4].about);
    this.about2 = (comida.adelgazar[4].about2);

  }
  Musculacion() {
    this.about = (comida.muscular[4].about);
    this.about2 = (comida.muscular[4].about2);
  }
  ...
  guardarKcal() {
    firebase.auth().onAuthStateChanged((user) => {
      const uid = user.uid;
      var docRef = this.db.collection("users").doc(uid);
      this.db.collection('users').doc(uid).collection("Kcal").doc("Cena").update({
  ...
  editarBase2() {
    firebase.auth().onAuthStateChanged((user) => {
      const uid = user.uid;
      var docRef = this.db.collection("users").doc(uid);
      this.db.collection('users').doc(uid).collection("Comidas").doc("Cena").update({
